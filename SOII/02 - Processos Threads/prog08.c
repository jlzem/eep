/* 
   Código demonstrativo para a aula de Sistemas Operacionais II,
   Curso de Ciência da Computação e Engenharia da Computação
   EEP.
*/

#include <stdio.h>
#include <stdlib.h>
#include <unistd.h>
#include <pthread.h>
#include <string.h>

char mensagem[80]="MSG do Proc. Pai";

void* filho01 () {
   printf("[Filho01] Iniciando Execucao.\n");
   printf("[Filho01] O conteudo da variavel MENSAGEM e: %s\n", mensagem);
   strcpy(mensagem,"MSG do Proc. Filho 01");
   sleep(5);
   printf("[Filho01] O conteudo da variavel MENSAGEM e: %s\n", mensagem);
   printf("[Filho01] Encerrando Execucao.\n");	
   pthread_exit(0);
}

void* filho02 () {
   printf("[Filho02] Iniciando Execucao.\n");
   printf("[Filho02] O conteudo da variavel MENSAGEM e: %s\n", mensagem);
   strcpy(mensagem,"MSG do Proc. Filho 02");
   sleep(5);
   printf("[Filho02] O conteudo da variavel MENSAGEM e: %s\n", mensagem);
   printf("[Filho02] Encerrando Execucao.\n");	
   pthread_exit(0);
}

int main(int argc, char *argv[]) {
  pthread_t	thread_id_01, thread_id_02;
  system("clear");
  printf("[Pai] Iniciando Execucao.\n");
  printf("[Pai] O conteudo da variavel MENSAGEM e: %s\n", mensagem);
  pthread_create(&thread_id_01,NULL,&filho01,NULL);
  pthread_create(&thread_id_02,NULL,&filho02,NULL);
  pthread_join(thread_id_01,NULL);
  pthread_join(thread_id_02,NULL);
  printf("[Pai] O conteudo da variavel MENSAGEM e: %s\n", mensagem);
  printf("[Pai] Encerrando Execucao.\n");  
  return 0;
} 
